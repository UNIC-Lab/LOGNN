...
def generate_layouts(user_nums, server_nums,args):
    ...
    for idx in range(len(server_nums)):
        ...
        # 信道归一化
        path_loss_mean = np.mean(u2s_path_loss)
        path_loss_var = np.sqrt(np.mean(np.square(u2s_path_loss)))
        u2s_path_loss_feat = (u2s_path_loss - path_loss_mean)/path_loss_var
        ...
        user_idx_feat = np.concatenate([user_idx_feat, user_idx_tasksize], axis=-1)
        server_idx_feat = np.concatenate([server_idx_feat, server_idx_comp], axis=-1)
        graph = build_graph(user_idx_tasksize, server_idx_comp, s2u_index, u2s_index, u2u_index, s2s_index, u2u_distances_idx, u2s_path_loss, u2s_path_loss_feat, env_len)
        graphs.append(graph)
